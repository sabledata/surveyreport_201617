...
The `r yr` survey was 42 days long and divided into three legs of 14, 16 and 14 days for a total of 34 fishing days. 
...
```{r SamplingWords, echo=FALSE}
   ...
   # -- other fish counts
     ...
     otherfish    <-   read.csv("c:/github/surveyreport/standaloneData/results17.csv",header=T)
     ...
     otherfish2    <-   read.csv("c:/github/surveyreport/standaloneData/results18.csv",header=T)      
...
```{r SablefishAgesText, echo=FALSE}
   ...
   agedat      <-    read.csv("c:/github/surveyreport/standaloneData/results19.csv",header=T)
   
   oldageF     <-  "exec procRReport_Survey_oldAgeFish 2"  # -- oldest female text caption
   #oldageFem   <-  GetSQLData(oldageF,"Sablefish") 
   oldageFem    <-    read.csv("c:/github/surveyreport/standaloneData/results20.csv",header=T)
   
   oldageM     <-  "exec procRReport_Survey_oldAgeFish 1"  # -- oldest male text caption
   #oldageMale  <-  GetSQLData(oldageM,"sablefish") 
   oldageMale  <-    read.csv("c:/github/surveyreport/standaloneData/results21.csv",header=T)   
...
```{r SablefishOceanText, echo=FALSE}
     ...
     whatTraps <-  read.csv("c:/github/surveyreport/standaloneData/results22.csv",header=T) 
     
     seabrd    <-  "select * from SEABIRD_ReportLinePlot"
     #seabird   <-  GetSQLData(seabrd ,"Sablefish") 
     seabird   <-  read.csv("c:/github/surveyreport/standaloneData/results23.csv",header=T)
...
SBE 39 recorders have been placed on survey fishing sets since 2006. In the shallow waters, the lowest average temperature was `r round(lavtmin,1)` $^\circ$C (`r lavtyr`) within the `r lavtband1`$^\circ$ - `r lavtband2`$^\circ$ latitude band.  The highest average temperature was `r round(lavtmax,1)` $^\circ$C (`r lavtyrmx`) in the southern `r lavtband1mx`$^\circ$ - `r lavtband2mx`$^\circ$ latitude band. Moving into the mid-depth waters, from 458-823 meters, the lowest average temperature was `r round(mavt,1)` $^\circ$C (`r mavtyr`) within the `r mavtband1`$^\circ$ -`r mavtband2` $^\circ$ latitude band.  The highest average temperature was `r round(mavtmx,1)` $^\circ$C (`r mavtyrmx`) in the southern `r mavtband1mx`$^\circ$ - `r mavtband2mx`$^\circ$ latitude band.  In the deepest waters, the lowest average temperature of `r round(davt,1)` $^\circ$C  (`r davtyr`) was found in the `r davtband1`$^\circ$ - `r davtband2`$^\circ$ latitude band and the highest average temperature was `r round(davtmx,1)`$^\circ$C (`r davtyrmx`) in the southern `r davtband1mx`$^\circ$ -`r davtband2mx`$^\circ$ latitude band (Figure \@ref(fig:figure16)).
...
```{r acknowledge, echo=FALSE}
    ...
    credits    <-  read.csv("c:/github/surveyreport/standaloneData/results24.csv",header=T)
    ...
    credits2  <-  read.csv("c:/github/surveyreport/standaloneData/results25.csv",header=T)
